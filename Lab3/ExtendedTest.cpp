...
void testIteratorKey() {
    cout<<"Test Iterator Keys\n";
    SortedMultiMap map(asc);
    ...
    // Create an iterator and iterate over the keys
    SMMIteratorKey iterator = map.iteratorKey();
    int expectedKeys[] = {1, 1, 2, 2, 3};
    int index = 0;

    while (iterator.valid()) {
        TKey key = iterator.getCurrent();
        assert(key == expectedKeys[index]);
        iterator.next();
        index++;
    }
}
